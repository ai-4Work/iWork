...
| 触发类型 | 策略 | 参数 |
|---------|------|------|
| **被动触发（阈值）** | 上下文用量达到压缩阈值时自动压缩 | 压缩后保留 30%-50% token |
| **主动触发（轮次周期）** | 每 10-15 轮后周期性检查，重锚定目标 | 周期性重锚定，配合压缩后 token 估算决定是否进入卸载 |

阈值按模型配：`llm_model.compress_threshold_tokens`（绝对 token）。留空则按该行
`context_window × 比例`折算——build 80% / ask 55%，即上表默认值。窗口是模型能力声明，
阈值才是触发线，两者分开配：想让压缩早点发生不必去改小窗口。

任务类型影响阈值——调试任务设大一些（90%+ 窗口），简单问答可配小一些（50-60%）激进压缩。
