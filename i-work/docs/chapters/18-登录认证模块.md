...
- `password_hash IS NULL` 的账号（如 `default-user` 初始态）一律返回 `INVALID_CREDENTIALS`，不区分对待。
...
| 接口 | 方法 | 路径 | 说明 |
|---|---|---|---|
| 注册 | POST | /auth/register | 开放注册，只需 username + password；不返回 token，需显式登录。`display_name` 默认取 username |
...
>
> 注册的查重不能只靠「先 SELECT 再 INSERT」——并发下两个请求会同时通过。靠 `username` 唯一索引兜底，捕获 `IntegrityError` 转成 `USERNAME_TAKEN`。
...
>
> 新增的 `status` / `failed_login_count` 必须带 `server_default`（`'active'` / `0`），否则已有的 `default-user` 行会因 NOT NULL 迁移失败。`password_hash` 可空——NULL 即「不可密码登录」。
...
|---|---|---|
| 登录（AuthN） | 确定你是谁 | 一次性 |
| 鉴权依赖 | 认证前置：验签 + 注入身份，返回 `UUID`（与 `get_default_user_id` 同签名，28 处调用点只需换依赖名） | 每次请求 |
...
| 11.1 | 注册、登录、刷新、登出、登出所有设备、当前用户、修改密码 |
...
| 13.1 | 现有业务接口改用 `get_current_user` 鉴权（`routes.py` / `skill_routes.py` / `mcp_routes.py` 共 28 处 `get_default_user_id` 全部替换；替换完删掉 `get_default_user_id` 与 `app.state.default_user_id`，`seed_default_user` 保留） |
